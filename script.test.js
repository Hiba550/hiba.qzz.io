...
function buildDOM() {
  ...
  document.body.innerHTML = `
    ...
    <img loading="lazy" src="test.jpg" alt="Test image">
  `;
}
...
describe('writeTheme', () => {
  ...
  it('shows a toast and warns when localStorage is unavailable', () => {
    const { writeTheme } = loadScript();
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    expect(toast.textContent).toContain('theme set');
    spy.mockRestore();
    warnSpy.mockRestore();
  });
});
...
describe('flashLabel', () => {
  it('temporarily changes label and adds is-copied class', () => {
    const { flashLabel } = loadScript();
    const el = document.querySelector('[data-copy]');
    const toast = document.querySelector('[data-toast]');

    flashLabel(el, 'Temp', 'toast msg', 1000);

    expect(el.dataset.label).toBe('Temp');
    expect(el.classList.contains('is-copied')).toBe(true);
    expect(toast.textContent).toBe('toast msg');
  });

  it('restores original label after specified duration', () => {
    const { flashLabel } = loadScript();
    const el = document.querySelector('[data-copy]');

    flashLabel(el, 'Temp', 'msg', 1500);

    jest.advanceTimersByTime(1500);
    expect(el.dataset.label).toBe('Discord');
    expect(el.classList.contains('is-copied')).toBe(false);
  });

  it('restores empty string when no original label', () => {
    const { flashLabel } = loadScript();
    const el = document.querySelector('[data-copy]');
    delete el.dataset.label;

    flashLabel(el, 'X', 'msg', 500);

    jest.advanceTimersByTime(500);
    expect(el.dataset.label).toBe('');
  });
});
...
describe('showTab', () => {
  ...
  it('warns when history.replaceState throws', () => {
    const { showTab } = loadScript();
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    showTab('work', true);
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining('Could not update URL'),
      expect.any(Error)
    );
...
describe('image error handling', () => {
  it('hides image and shows notice on load error', () => {
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    loadScript();
    ...
    const notice = img.nextSibling;
    expect(notice.textContent).toBe('image could not be loaded.');
    warnSpy.mockRestore();
  });

  it('only handles error once per image', () => {
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    loadScript();
    const img = document.querySelector('img[loading]');
    ...
    // Count only notice paragraphs inserted after the image
    let count = 0;
    let sibling = img.nextSibling;
    while (sibling) {
      if (sibling.nodeType === 1 && sibling.tagName === 'P') count++;
...
describe('global error handlers', () => {
  it('shows toast on uncaught error', () => {
    ...
    const event = new ErrorEvent('error', { message: 'test error', error: new Error('test') });
    window.dispatchEvent(event);

    expect(toast.textContent).toBe('something went wrong. check the console for details.');
    expect(toast.classList.contains('is-visible')).toBe(true);
    errorSpy.mockRestore();
  });

  it('shows toast on unhandled promise rejection', () => {
    const errorSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
    loadScript();
    const toast = document.querySelector('[data-toast]');

    const event = new Event('unhandledrejection');
    event.reason = new Error('async fail');
    window.dispatchEvent(event);
